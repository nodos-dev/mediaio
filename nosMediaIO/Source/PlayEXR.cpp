...
struct PlayEXRNode : NodeContext
{
	std::mutex Mtx;
	...
	PlayEXRNode(nosFbNodePtr node) : NodeContext(node)
	{
	...
	void WorkerLoop()
	{
		...
		while (!Stop)
		{
			...
			++DecodeCount;
			if (!ok) nosEngine.LogE("PlayEXR: frame %d: %s", todo, err.c_str());
	...
	{
		nosEngine.WatchLog("PlayEXR ready-ahead",
			(std::to_string(readyAhead) + " / " + std::to_string(ahead)).c_str());
		nosEngine.WatchLog("PlayEXR cached frames", std::to_string(cacheSize).c_str());
		...
		if (LastExecTime.time_since_epoch().count() != 0)
		{
			...
			nosEngine.WatchLog("PlayEXR index rate (frames/s)", buf);
			snprintf(buf, sizeof(buf), "%.1f", EmaExecRate);
			nosEngine.WatchLog("PlayEXR exec rate (/s)", buf);
			snprintf(buf, sizeof(buf), "%.1f", EmaDecodeRate);
			nosEngine.WatchLog("PlayEXR decode rate (frames/s)", buf);
			snprintf(buf, sizeof(buf), "%.0f", EmaDecodeMs);
			nosEngine.WatchLog("PlayEXR decode avg (ms)", buf);
		}
		LastExecTime = now;
		LastDecodes = decodes;
...
nosResult RegisterPlayEXR(nosNodeFunctions* fn)
{
	NOS_BIND_NODE_CLASS(NOS_NAME_STATIC("PlayEXR"), PlayEXRNode, fn);
	return NOS_RESULT_SUCCESS;
}
