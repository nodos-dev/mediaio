...
std::optional<vkss::Resource> UploadColor(const ExrFrame& frame, nosUUID nodeId, nosGPUEvent* outEvent)
{
	...
	texInfo.Info.Texture.Width = uint32_t(frame.Width);
	texInfo.Info.Texture.Height = uint32_t(frame.Height);
	texInfo.Info.Texture.Format = frame.ColorFormat;
	texInfo.Info.Texture.FieldType = NOS_TEXTURE_FIELD_TYPE_PROGRESSIVE;

	auto texture = vkss::Resource::Create(texInfo, "EXR Texture");
	if (!texture)
		return std::nullopt;

	nosCmd cmd = vkss::BeginCmd(NOS_NAME("EXR Upload"), nodeId);
	nosVulkan->ImageLoad(cmd, const_cast<uint8_t*>(frame.Color.data()),
		nosVec2u(frame.Width, frame.Height), frame.ColorFormat, &*texture, nullptr);
	nosGPUEvent event = 0;
...
std::optional<vkss::Resource> UploadDepth(const ExrFrame& frame, nosUUID nodeId, nosGPUEvent* outEvent)
{
	...
	auto texture = vkss::Resource::Create(texInfo, "EXR Depth");
	if (!texture)
		return std::nullopt;

	nosCmd cmd = vkss::BeginCmd(NOS_NAME("EXR Depth Upload"), nodeId);
	nosVulkan->ImageLoad(cmd, const_cast<float*>(frame.Depth.data()),
		nosVec2u(frame.Width, frame.Height), NOS_FORMAT_R32_SFLOAT, &*texture, nullptr);
	nosGPUEvent event = 0;
	nosCmdEndParams endParams{ .ForceSubmit = true, .OutGPUEventHandle = &event };
	nosVulkan->End(cmd, &endParams);
	if (outEvent)
