...
struct ReadEXRSequenceNode : NodeContext
{
	...
	{
		const FileVector* files = params.GetPinValue<FileVector>(NOS_NAME_STATIC("Files"));
	...
	{
		SetPinObject(NOS_NAME_STATIC("Texture"), e.Color);
		if (e.HasDepth && e.Depth.IsValid())
			SetPinObject(NOS_NAME_STATIC("Depth"), e.Depth);
		if (e.Metadata)
